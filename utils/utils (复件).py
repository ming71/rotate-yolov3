...
def build_targets(model, targets, hyp):
    ...
    # 防止所有layer都没有合适的box时可以用最大的替换
    alternative_indice = []     # 记录可能的候选anchor index
    alternative_iou = []        # 记录可能的候选anchor的iou
    alternative_gt = []         # 记录可能的候选anchor的对应的gt
    no_anchor_flag = ft([0 for i in range(nt)])
    ...
    if reject:
        ...
        for lid, mask_j in enumerate(j):
            ...
            tcls[lid] = tcls[lid][mask_j]
            av[lid]   = av[lid][mask_j]
            indices[lid][0]  = indices[lid][0][mask_j]
            indices[lid][1]  = indices[lid][1][mask_j]
            indices[lid][2]  = indices[lid][2][mask_j]
            indices[lid][3]  = indices[lid][3][mask_j]
