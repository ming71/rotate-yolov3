...
if __name__ == '__main__':
    ...
    # parser.add_argument('--data', type=str, default='data/tiny.data', help='coco.data file path')
    ...
    #     np.savetxt('benchmark.txt', y, fmt='%10.4g')  # y = np.loadtxt('study.txt')
        ...
        for i in x:
            t = time.time()
            r = test(opt.cfg, opt.data, opt.weights, opt.batch_size, opt.img_size, 
                     i, opt.nms_thres, opt.save_json, hyp)[0]
            y.append(r + (time.time() - t,))
